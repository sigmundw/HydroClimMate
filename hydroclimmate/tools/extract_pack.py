#!/usr/bin/env python3
"""extract_pack.py -- mechanical, deterministic extractors for the hrldas-noahmp
knowledge pack's catalogs. Every subcommand reads the pinned HRLDAS/Noah-MP source tree
given by --source-root and writes one catalogs/*.yaml file (see SCHEMA.md). Nothing here
is hand-typed: each fact is produced by a regex/parse pass over the actual source files,
and carries `where` = {file, line, commit} pointing at the exact source line(s) it came
from.

Pure stdlib (re, json, pathlib, hashlib, argparse) -- no numpy/netCDF4 needed to run
these; that dependency is confined to validate_catalogs.py, which confronts the output
of these extractors against real files.
...
Usage:
  extract_pack.py outputs         --source-root ROOT --out catalogs/outputs.yaml
  extract_pack.py restart         --source-root ROOT --out catalogs/restart.yaml
  extract_pack.py forcing         --source-root ROOT --out catalogs/forcing.yaml
  extract_pack.py setup           --source-root ROOT --out catalogs/setup.yaml
  extract_pack.py namelist        --source-root ROOT --out catalogs/namelist.yaml
  extract_pack.py physics-options --source-root ROOT --out catalogs/physics_options.yaml
  extract_pack.py parameters      --source-root ROOT --out catalogs/parameters.yaml
  extract_pack.py constants       --source-root ROOT --out catalogs/constants.yaml
  extract_pack.py urban-path      --source-root ROOT --out catalogs/urban_path.yaml
  extract_pack.py all             --source-root ROOT --out-dir catalogs/
...
def write_yaml(obj, out_path, command_name):
    """Pack YAML layers a person or agent reads: strict-subset YAML (see _miniyaml.py
    and SCHEMA.md), with a generated-file header. `obj` must already be a plain dict of
    JSON-safe values (dict/list/str/int/float/bool/None) -- exactly what these extractors
    already produce."""
    out_path.parent.mkdir(parents=True, exist_ok=True)
    header = [
    ...
    out_path.write_text(_miniyaml.dump(obj, header_lines=header))


def main(argv=None):
    parser = argparse.ArgumentParser(description=__doc__, formatter_class=argparse.RawDescriptionHelpFormatter)
    sub = parser.add_subparsers(dest="command", required=True)
    for name in list(CATALOGS) + ["all"]:
        p = sub.add_parser(name)
        p.add_argument("--source-root", required=True, help="Root of the pinned HRLDAS+noahmp checkout")
        if name == "all":
            p.add_argument("--out-dir", required=True, help="catalogs/ directory to write into")
        else:
            p.add_argument("--out", required=True, help="Output YAML path (pack layer; MANIFEST.json under 'all' stays JSON)")
    ...
    if args.command == "all":
        ...
        written = {}
        for name, func in CATALOGS.items():
            obj = func(root)
            out_path = out_dir / FILENAME[name]
            write_yaml(obj, out_path, name)
            written[FILENAME[name]] = {"count": obj.get("count"), "sha256": sha256_of(out_path)}
            print(f"wrote {out_path} ({obj.get('count')} entries)")
        manifest = {
            "pack": "hrldas-noahmp", "generator": "tools/extract_pack.py all",
    ...
    obj = CATALOGS[args.command](root)
    write_yaml(obj, Path(args.out), args.command)
